...
def plane_net(src, trg, src_adj, trg_adj, train_mask, valid_mask, test_mask, src_label, label, split_idx, method):
    # Model
    ...
    idx, x1, x2, x3 = 0, [], [], []
    pred, trg_out, trg_y, loss = 0, 0, 0, 0
    stop = False
    ...
    for i in range(3000):
        if stop:
            break
        if method == 'mlp':
            trg_out = F.log_softmax(model(x_trg), dim=1)
            trg_y = torch.tensor(label, dtype=torch.long, device=device)
            loss = F.nll_loss(trg_out[train_mask], trg_y[train_mask])
        elif method == 'gcn':
            edge_index = torch.tensor(dense_to_sparse(trg_adj)[0]).to(device)
            trg_out = F.log_softmax(model(x_trg, edge_index), dim=1)
            trg_y = torch.tensor(label, dtype=torch.long, device=device)
            loss = F.nll_loss(trg_out[train_mask], trg_y[train_mask])
        elif method == 'gat':
            edge_index = torch.tensor(dense_to_sparse(trg_adj)[0]).to(device)
            ...
            src_out, trg_out, domain_loss = model(x_src, x_trg, device)
            src_out, trg_out = F.log_softmax(src_out, dim=1), F.log_softmax(trg_out, dim=1)
            src_y = torch.tensor(src_label, dtype=torch.long, device=device)
            trg_y = torch.tensor(label, dtype=torch.long, device=device)
            loss = F.nll_loss(src_out, src_y) + F.nll_loss(trg_out[train_mask], trg_y[train_mask]) + domain_loss * .2
        elif method == 'mmt':
            src_out, trg_out = model(x_src, x_trg, device)
            src_out, trg_out = F.log_softmax(src_out, dim=1), F.log_softmax(trg_out, dim=1)
            src_y = torch.tensor(src_label, dtype=torch.long, device=device)
            trg_y = torch.tensor(label, dtype=torch.long, device=device)
            loss = F.nll_loss(src_out, src_y) + F.nll_loss(trg_out[train_mask], trg_y[train_mask])
        elif method == 'ser':
            src_out, trg_out, domain_loss = model(x_src, x_trg, device)
            src_out, trg_out = F.log_softmax(src_out, dim=1), F.log_softmax(trg_out, dim=1)
            src_y = torch.tensor(src_label, dtype=torch.long, device=device)
            ...
            src_pred, trg_pred, domain_loss = model(x_src, dense_to_sparse(src_adj), x_trg, dense_to_sparse(trg_adj), device)
